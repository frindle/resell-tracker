...
test('runWaitlist: WAIT fires neither hook; summary buckets are correct', async () => {
  const calls: string[] = [];
  const hooks = {
    submit: (c: WaitlistCard) => { calls.push(`submit:${c.id}`); },
    onExpire: (c: WaitlistCard) => { calls.push(`expire:${c.id}`); },
  };
...
test('runWaitlist: a throwing submit hook records exactly one {id,message} error and the run continues', async () => {
  const hooks = {
    submit: (c: WaitlistCard) => { if (c.id === 'boom') throw new Error('gateway down'); },
    onExpire: () => {},
  };
  const s = await runWaitlist(
...
test('runWaitlist: a throwing onExpire hook is recorded the same way; bare-string throws stay readable', async () => {
  const hooks = {
    submit: () => {},
    onExpire: (c: WaitlistCard) => { if (c.id === 'x1') throw new Error('db lock'); if (c.id === 'x2') throw 'raw string failure'; },
  };
  const s = await runWaitlist(
