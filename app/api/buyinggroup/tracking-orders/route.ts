...
export async function GET() {
  ...
  const result: Record<string, { id: number; orderNumber: string | null; itemDescription: string | null; salePrice: number | null; bgExpectedPayout: number | null; platform: string }[]> = {};
  for (const o of orders) {
    if (!o.trackingNumbers) continue;
    for (const t of o.trackingNumbers.split(',').map(s => s.trim().replace(/\D/g, '')).filter(Boolean)) {
      if (!result[t]) result[t] = [];
      result[t].push({ id: o.id, orderNumber: o.orderNumber, itemDescription: o.itemDescription, salePrice: o.salePrice, bgExpectedPayout: o.bgExpectedPayout, platform: o.platform });
    }
  }
