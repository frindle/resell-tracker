...
export async function GET(req: NextRequest) {
  ...
  const [orders, total] = await Promise.all([
    prisma.order.findMany({
      where,
      include: { buyer: true, card: { include: { merchantRates: true } }, giftCards: { select: { ccSubmittedAt: true, cardNumber: true } }, commitmentLinks: { select: { id: true, quantity: true } }, bfmrLinks: { select: { id: true, quantity: true, trackingNumber: true, reservation: { select: { status: true, qty: true, trackingNumber: true, submittedShipments: { select: { trackingNumber: true, qty: true } } } } } }, returns: { select: { status: true, quantity: true } } },
