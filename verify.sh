#!/usr/bin/env bash
# verify for: rt-group-address-match (--ts-runner node-test)
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'lib/groupAddressMatch.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: lib/groupAddressMatch.ts parses"
  ...
  echo "  FAIL: lib/groupAddressMatch.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'lib/groupAddressMatch\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in lib/groupAddressMatch.ts"; grep -E 'lib/groupAddressMatch\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE lib/groupAddressMatch.ts (not this task's) -- passing type gate"
fi
