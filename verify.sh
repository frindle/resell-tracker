#!/usr/bin/env bash
# verify for: rt-address-normalize (--ts-runner node-test)
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'lib/retailerAddressImport.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: lib/retailerAddressImport.ts parses"
  ...
  echo "  FAIL: lib/retailerAddressImport.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'lib/retailerAddressImport\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in lib/retailerAddressImport.ts"; grep -E 'lib/retailerAddressImport\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE lib/retailerAddressImport.ts (not this task's) -- passing type gate"
fi
