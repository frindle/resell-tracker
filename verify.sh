#!/usr/bin/env bash
# verify for: rt-dashboard-pl-unsubmitted-cc (--ts-runner node-test)
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'app/page.tsx' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: app/page.tsx parses"
  ...
  echo "  FAIL: app/page.tsx does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'app/page\.tsx[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in app/page.tsx"; grep -E 'app/page\.tsx[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE app/page.tsx (not this task's) -- passing type gate"
fi
