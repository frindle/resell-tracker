#!/usr/bin/env bash
# verify for: rt-bfmr-409-logging-p1 (--ts-runner node-test)
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'app/api/bfmr/submit-reservation-tracking/route.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: app/api/bfmr/submit-reservation-tracking/route.ts parses"
  ...
  echo "  FAIL: app/api/bfmr/submit-reservation-tracking/route.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'app/api/bfmr/submit\-reservation\-tracking/route\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in app/api/bfmr/submit-reservation-tracking/route.ts"; grep -E 'app/api/bfmr/submit\-reservation\-tracking/route\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE app/api/bfmr/submit-reservation-tracking/route.ts (not this task's) -- passing type gate"
fi
