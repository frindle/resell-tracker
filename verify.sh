#!/usr/bin/env bash
# verify for: rt-reservation-remaining (--ts-runner node-test)
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'lib/reservationRemaining.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: lib/reservationRemaining.ts parses"
  ...
  echo "  FAIL: lib/reservationRemaining.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'lib/reservationRemaining\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in lib/reservationRemaining.ts"; grep -E 'lib/reservationRemaining\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE lib/reservationRemaining.ts (not this task's) -- passing type gate"
fi
