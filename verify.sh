#!/usr/bin/env bash
# verify for: amazon-iris-payment-fix (--ts-runner node-test)
...
echo "=== env bootstrap ==="
# The sidecar throws at IMPORT time when TRACKER_URL is unset
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'sidecar/src/amazon.js' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: sidecar/src/amazon.js parses"
  ...
  echo "  FAIL: sidecar/src/amazon.js does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'sidecar/src/amazon\.js[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in sidecar/src/amazon.js"; grep -E 'sidecar/src/amazon\.js[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE sidecar/src/amazon.js (not this task's) -- passing type gate"
fi
