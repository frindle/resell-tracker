#!/usr/bin/env bash
# verify for: rt-bfmr-sync-scope-wiring-v2 (--ts-runner node-test)
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'lib/bfmrSyncTrigger.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: lib/bfmrSyncTrigger.ts parses"
  ...
  echo "  FAIL: lib/bfmrSyncTrigger.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'lib/bfmrSyncTrigger\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in lib/bfmrSyncTrigger.ts"; grep -E 'lib/bfmrSyncTrigger\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE lib/bfmrSyncTrigger.ts (not this task's) -- passing type gate"
fi
