#!/usr/bin/env bash
# verify for: bfmr-stale-link-migration
...
echo "=== target parses ==="
# The target is TypeScript -- python ast.parse can never read it. Node 26
# strips types and imports the module directly; a syntax error in the file
# surfaces as a non-zero exit here.
if node --input-type=module -e "import('./lib/bfmrJoin.ts').then(()=>process.exit(0),()=>process.exit(1))" >/dev/null 2>&1; then
  echo "  ok: lib/bfmrJoin.ts parses"
else
  echo "  FAIL: lib/bfmrJoin.ts does not parse"; fails=$((fails+1))
fi
