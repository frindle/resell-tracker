#!/usr/bin/env bash
# verify for: bfmr-paid-rollup-overcount (--ts-runner node-test)
...
TEST_FILES="lib/bfmrPaidRollup.test.ts"
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'lib/bfmr.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: lib/bfmr.ts parses"
  ...
  echo "  FAIL: lib/bfmr.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'lib/bfmr\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in lib/bfmr.ts"; grep -E 'lib/bfmr\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE lib/bfmr.ts (not this task's) -- passing type gate"
fi
