#!/usr/bin/env bash
# verify for: rt-order919-address-resync (--ts-runner node-test)
...
TEST_FILES="lib/orderFieldSync.test.ts"
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'lib/orderFieldSync.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: lib/orderFieldSync.ts parses"
  ...
  echo "  FAIL: lib/orderFieldSync.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
