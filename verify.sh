#!/usr/bin/env bash
# verify for: rt-walmart555-tracking-fix (--ts-runner node-test)
...
# sidecar/src/walmart.js lives under a sub-package with its OWN node_modules in the source;
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'sidecar/src/walmart.js' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: sidecar/src/walmart.js parses"
  ...
  echo "  FAIL: sidecar/src/walmart.js does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'sidecar/src/walmart\.js[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in sidecar/src/walmart.js"; grep -E 'sidecar/src/walmart\.js[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE sidecar/src/walmart.js (not this task's) -- passing type gate"
fi
