#!/usr/bin/env bash
# verify for: rt-amazon-card-lastfours (--ts-runner node-test)
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'app/api/import/route.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: app/api/import/route.ts parses"
  ...
  echo "  FAIL: app/api/import/route.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'app/api/import/route\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in app/api/import/route.ts"; grep -E 'app/api/import/route\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE app/api/import/route.ts (not this task's) -- passing type gate"
fi
