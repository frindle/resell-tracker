#!/usr/bin/env bash
# verify for: bfmr-sync-orders-paginate (--ts-runner node-test)
...
TEST_FILES="verify.sync-orders.test.mts"
...
if [ "$HAS_PATHS" = "1" ]; then
  RUNNER="$TSX --tsconfig ./verify.tsconfig.json --test --test-reporter=tap"
  echo "  runner: tsx --tsconfig ./verify.tsconfig.json --test --test-reporter=tap (tsconfig paths present -- resolves @/ aliases + next/headers stub)"
else
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'app/api/bfmr/sync-orders/route.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: app/api/bfmr/sync-orders/route.ts parses"
  ...
  echo "  FAIL: app/api/bfmr/sync-orders/route.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'app/api/bfmr/sync\-orders/route\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in app/api/bfmr/sync-orders/route.ts"; grep -E 'app/api/bfmr/sync\-orders/route\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE app/api/bfmr/sync-orders/route.ts (not this task's) -- passing type gate"
fi
...
echo "=== harness: next/headers stub + verify tsconfig + temp DB ==="
# The route calls getSessionUserId() -> next/headers cookies(), which THROWS
# outside a Next request scope. Redirect next/headers to a no-op stub via a
# verify-local tsconfig (tsx honours tsconfig `paths` for bare specifiers, proven
# on this repo). This is framework harness scaffolding, NOT the route logic and
# NOT lib/bfmr -- the transport boundary we stub is globalThis.fetch, in the test.
cat > ./.verify-next-headers.ts <<'STUB'
// Test stub: no Next request scope in a unit test. The acting user is supplied
// via the X-Extension-User-Id header instead of a session cookie.
export async function cookies() { return { get(_k: string) { return undefined; } } as any; }
export async function headers() { return new Map() as any; }
STUB
# extends the repo tsconfig and adds ONLY the next/headers redirect (+ @/*).
cat > ./verify.tsconfig.json <<TSCFG
{
  ...
  "compilerOptions": {
    ...
    "paths": {
      "@/*": ["./*"],
      "next/headers": ["./.verify-next-headers.ts"]
    }
  }
}
TSCFG
# Dedicated throwaway SQLite DB so getSetting() returns creds and the route runs
# end to end -- NEVER the repo's dev.db (the test also refuses any other DB).
TESTDB="$PWD/.verify-sync-orders.db"
export DATABASE_URL="file:$TESTDB"
rm -f "$TESTDB"
if npx --yes prisma migrate deploy >/tmp/_verify_migrate.$$.log 2>&1; then
  echo "  ok: temp DB migrated ($TESTDB)"
else
  echo "  FAIL: prisma migrate deploy on temp DB failed"; tail -20 /tmp/_verify_migrate.$$.log; fails=$((fails+1))
fi
rm -f /tmp/_verify_migrate.$$.log
