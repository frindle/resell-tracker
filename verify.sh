#!/usr/bin/env bash
# verify for: rt-emailsync-imap-configurable (--ts-runner node-test)
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'lib/emailSync.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: lib/emailSync.ts parses"
  ...
  echo "  FAIL: lib/emailSync.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'lib/emailSync\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in lib/emailSync.ts"; grep -E 'lib/emailSync\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE lib/emailSync.ts (not this task's) -- passing type gate"
fi
