#!/usr/bin/env bash
# verify for: rt-autobuy-decision (--ts-runner node-test)
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'lib/autoBuyDecision.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: lib/autoBuyDecision.ts parses"
  ...
  echo "  FAIL: lib/autoBuyDecision.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'lib/autoBuyDecision\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in lib/autoBuyDecision.ts"; grep -E 'lib/autoBuyDecision\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE lib/autoBuyDecision.ts (not this task's) -- passing type gate"
fi
