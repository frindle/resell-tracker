#!/usr/bin/env bash
# verify for: bfmr-sync-reservations-500-guard (--kind expression)
...
# broken target.
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'app/api/bfmr/sync-reservations/route.ts' 2>/tmp/_verify_parse.$$.log; then
  ...
  echo "  FAIL: app/api/bfmr/sync-reservations/route.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
# edited (app/api/bfmr/sync-reservations/route.ts). Pre-existing errors elsewhere WARN and pass.
if $TSC --noEmit -p tsconfig.json >/tmp/_verify_tsc.$$.log 2>&1; then
  echo "  ok: tsc --noEmit clean"
elif grep -qE 'app/api/bfmr/sync\-reservations/route\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in app/api/bfmr/sync-reservations/route.ts"; grep -E 'app/api/bfmr/sync\-reservations/route\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE app/api/bfmr/sync-reservations/route.ts (not this task's) -- passing type gate"
fi
