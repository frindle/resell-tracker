#!/usr/bin/env bash
# verify for: rt-walmart-delivered-signal-wiring-v2 (--ts-runner node-test)
...
# --- sub-package node_modules (env parity) ---
# sidecar/src/walmartDetailSignals.js lives under a sub-package with its OWN node_modules in the source;
# the repo-root node_modules symlink does not cover it. Link the sub-package's
# deps from the source (same HEAD, same lock) -- do NOT clobber an existing path.
if [ ! -e sidecar/node_modules ]; then
  if [ -d '/Users/penn/Desktop/GitHub Projects/resell-tracker/sidecar/node_modules' ]; then
    mkdir -p "$(dirname sidecar/node_modules)"
    ln -s '/Users/penn/Desktop/GitHub Projects/resell-tracker/sidecar/node_modules' sidecar/node_modules && echo "  ok: linked sidecar/node_modules -> source"
  else
    echo "  WARN: sub-package deps '/Users/penn/Desktop/GitHub Projects/resell-tracker/sidecar/node_modules' not installed in the source -- run npm ci there"
  fi
else
  echo "  ok: sidecar/node_modules already present"
fi
...
echo "=== target parses ==="
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'sidecar/src/walmartDetailSignals.js' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: sidecar/src/walmartDetailSignals.js parses"
  ...
  echo "  FAIL: sidecar/src/walmartDetailSignals.js does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
elif grep -qE 'sidecar/src/walmartDetailSignals\.js[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in sidecar/src/walmartDetailSignals.js"; grep -E 'sidecar/src/walmartDetailSignals\.js[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE sidecar/src/walmartDetailSignals.js (not this task's) -- passing type gate"
fi
