...
### (root)
- `check_literals.py`: TARGET (const), LITERALS (const)
...
- `lib/bfmr.ts`: BFMR_STATUS_RANK (const), BFMR_TERMINAL_STATUSES (const), deriveBfmrStatus (fn), testConnection (fn), getMyTracker (fn), getMyTrackerAll (fn), updateTracker (fn), getDeals (fn), getDeal (fn), getActiveReservations (fn), getShipmentStatus (fn), getInsuredShipments (fn), ...(+2 more)
...
- `app/settings/page.tsx:994` -> `{users.length > 0 && (`
