# TASK: diag-bfmr-overalloc-guard (diagnosis)

## Confirmed symptom (observed, not suspected)

CONFIRMED live via API: Amazon order 907 (111-3026367-4750648) accrued 4 OrderBfmrLink rows summing to 5 units when only 3 iPads exist. Reservation 165063 has qty=2 but had links 152(qty2)+154(qty1)=3 units linked to it (exceeds its qty); separately links 158 and 166 both claimed the SAME tracking TBA334443592549. I deleted links 154+158 to restore the correct 3.

## The question to answer

In the OrderBfmrLink creation/matching code path, where and why is there no guard preventing (a) the sum of a reservation's linked quantities from exceeding that reservation.qty, and (b) two OrderBfmrLink rows from claiming the same trackingNumber? Name the file:line where a link is created/updated without these checks.
