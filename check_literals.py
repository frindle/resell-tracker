...
TARGET = pathlib.Path('lib/bfmrSyncTrigger.ts')

# frozen from TASK.md -- re-run `ollama-dispatch-scaffold --freeze-literals .`
LITERALS = [[None, 'BfmrSyncTrigger'], [None, 'SYNC_TRIGGER_SCOPES'], [None, 'scopeForSyncTrigger'], [None, 'import type { BfmrSyncScope }'], [None, 'Object.freeze'], [None, 'order-open'], [None, 'manual'], [None, 'scheduled'], [None, 'pending'], [None, "'all'"], ['app/api/bfmr/sync-reservations/route.ts', 'resolveBfmrSyncPlan(scopeForSyncTrigger('], ['app/api/bfmr/sync-reservations/route.ts', 'plan.runWebBackfill && needsWebBackfill.length > 0'], ['components/BfmrReservationLinker.tsx', "trigger: 'order-open'"]]
