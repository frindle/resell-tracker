...
TARGET = pathlib.Path('app/api/bfmr/submit-reservation-tracking/route.ts')

# frozen from TASK.md -- re-run `ollama-dispatch-scaffold --freeze-literals .`
LITERALS = [[None, "import { logApiError } from '@/lib/apiErrorLog';"], [None, 'void logApiError({'], [None, 'export function findMissingBfmrId('], [None, 'has no bfmrOrderId — cannot submit tracking'], [None, 'has no myTrackerId — cannot submit tracking']]
