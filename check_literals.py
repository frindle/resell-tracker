...
LITERALS = [['app/api/bfmr/sync-reservations/route.ts', "resolveStaleReservationLinkMigrations } from '@/lib/bfmrJoin'"], ['app/api/bfmr/sync-reservations/route.ts', 'orderLinks: { some: {} }'], ['app/api/bfmr/sync-reservations/route.ts', 'orderLinks: { none: {} }'], ['app/api/bfmr/sync-reservations/route.ts', 'const staleLinkMigrations = resolveStaleReservationLinkMigrations(bareLinkedRows, liveUnlinkedRows);'], ['app/api/bfmr/sync-reservations/route.ts', 'await prisma.orderBfmrLink.updateMany({ where: { reservationId: m.fromId }, data: { reservationId: m.toId } });'], ['app/api/bfmr/sync-reservations/route.ts', 'staleLinkMigrations: staleLinkMigrations.length,'], ['app/api/bfmr/sync-reservations/route.ts', '...(staleLinkMigrations.length ? { staleLinkMigrationSamples: staleLinkMigrations.slice(0, 10) } : {}),']]
